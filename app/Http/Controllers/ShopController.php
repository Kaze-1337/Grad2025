...
class ShopController extends Controller
{
    public function index(Request $request){
        ...
        $products = Product::where(function($query) use ($f_brands) {
            if (!empty($f_brands)) {
                $brandArray = array_filter(explode(',', $f_brands)); // lọc bỏ phần tử rỗng
                if (!empty($brandArray)) {
            ...
            if (!empty($f_categories)) {
                $categoryArray = array_filter(explode(',', $f_categories)); // lọc bỏ phần tử rỗng
    ...
    public function product_details($product_slug){
        $product = Product::with('sizes')->where('slug',$product_slug)->first();
