        ...
        <img src="images/upload/upload-1.png" alt="">
                ...
                <div class="cols gap22">
                    <fieldset class="name">
                        ...
                        <div class="size-checkboxes mb-10">
                    ...
                    @error('sizes')<span class="alert alert-danger">{{ $message }}</span>@enderror
                </div>
                ...
                <div class="cols gap22">
                    ...
                    @error('featured')<span class="alert alert-danger">{{ $message }}</span>@enderror
...
<script>
    $(function() {
        // Xử lý sự kiện khi người dùng chọn file cho ảnh chính
        $("#myFile").on("change", function(e) {
            const [file] = this.files; // Lấy file được chọn
            if (file) {
                // Cập nhật thuộc tính src của thẻ img và hiển thị preview
                $("#imgpreview img").attr('src', URL.createObjectURL(file));
                $("#imgpreview").show(); // Hiển thị div chứa ảnh preview
            }
        });

        // Xử lý sự kiện khi người dùng chọn file cho gallery
        $("#gFile").on("change", function(e) {
            const gphotos = this.files; // Lấy danh sách file được chọn
            $.each(gphotos, function(key, file) {
                if (file) {
                    // Thêm ảnh preview vào container #galUpload
                    $("#galUpload").prepend('<div class="gitems"><img src="' + URL.createObjectURL(file) + '" alt=""></div>');
                }
            });
        });
    ...
    function StringToSlug(Text) {
        return Text.toLowerCase()
            .replace(/[^\w ]+/g, "") // Loại bỏ ký tự không phải chữ và số
